 ...
 *  http://averta.net
 */
"function"!=typeof Object.create&&(Object.create=function(obj){function F(){}return F.prototype=obj,new F}),/*! 
 * 
 ...
 * Copyright 2015 Metafizzy
 */
function(window,factory){"use strict";"function"==typeof define&&define.amd?define(["outlayer/outlayer","get-size/get-size","matches-selector/matches-selector","fizzy-ui-utils/utils","isotope/js/item","isotope/js/layout-mode","isotope/js/layout-modes/masonry","isotope/js/layout-modes/justify-rows","isotope/js/layout-modes/fit-rows","isotope/js/layout-modes/vertical"],function(Outlayer,getSize,matchesSelector,utils,Item,LayoutMode){return factory(window,Outlayer,getSize,matchesSelector,utils,Item,LayoutMode)}):"object"==typeof exports?module.exports=factory(window,require("outlayer"),require("get-size"),require("desandro-matches-selector"),require("fizzy-ui-utils"),require("./item"),require("./layout-mode"),require("./layout-modes/masonry"),require("./layout-modes/fit-rows"),require("./layout-modes/justify-rows"),require("./layout-modes/vertical")):window.Isotope=factory(window,window.Outlayer,window.getSize,window.matchesSelector,window.fizzyUIUtils,window.Isotope.Item,window.Isotope.LayoutMode)}(window,function(window,Outlayer,getSize,matchesSelector,utils,Item,LayoutMode){function getItemSorter(sortBys,sortAsc){return function(itemA,itemB){for(var i=0,len=sortBys.length;len>i;i++){var sortBy=sortBys[i],a=itemA.sortData[sortBy],b=itemB.sortData[sortBy];if(a>b||b>a){var isAscending=void 0!==sortAsc[sortBy]?sortAsc[sortBy]:sortAsc,direction=isAscending?1:-1;return(a>b?1:-1)*direction}}return 0}}var jQuery=window.jQuery,trim=String.prototype.trim?function(str){return str.trim()}:function(str){return str.replace(/^\s+|\s+$/g,"")},docElem=document.documentElement,getText=docElem.textContent?function(elem){return elem.textContent}:function(elem){return elem.innerText},Isotope=Outlayer.create("isotope",{layoutMode:"masonry",isJQueryFiltering:!0,sortAscending:!0,pagination:!1,inPage:20,page:1,useImagesLoaded:!0,lazyload:!1,resizeTransition:!0});Isotope.Item=Item,Isotope.LayoutMode=LayoutMode,Isotope.prototype._create=function(){this.itemGUID=0,this._sorters={},this._getSorters(),Outlayer.prototype._create.call(this),this.modes={},this.filteredItems=this.items,this.sortHistory=["original-order"];for(var name in LayoutMode.modes)this._initLayoutMode(name)},Isotope.prototype.reloadItems=function(){this.itemGUID=0,Outlayer.prototype.reloadItems.call(this)},Isotope.prototype._itemize=function(){for(var items=Outlayer.prototype._itemize.apply(this,arguments),i=0,len=items.length;len>i;i++){var item=items[i];item.id=this.itemGUID++}return this._updateItemsSortData(items),items},Isotope.prototype._initLayoutMode=function(name){var Mode=LayoutMode.modes[name],initialOpts=this.options[name]||{};this.options[name]=Mode.options?utils.extend(Mode.options,initialOpts):initialOpts,this.modes[name]=new Mode(this)},Isotope.prototype.layout=function(){return!this._isLayoutInited&&this.options.isInitLayout?void this.arrange():void this._layout()},Isotope.prototype._layout=function(){var isInstant=this._getIsInstant();this._resetLayout(),this._manageStamps(),this.layoutItems(this.filteredItems,isInstant),this._isLayoutInited=!0};var _layoutItems=Isotope.prototype.layoutItems;Isotope.prototype.layoutItems=function(items,isInstant){this._beforeLayout(items,isInstant),_layoutItems.apply(this,arguments)},Isotope.prototype.arrange=function(opts){this.option(opts),this._getIsInstant();var filtered=this._filter(this.items);if(this.filteredItems=filtered.matches,this.notPaginatedItems=this.filteredItems,this._sort(),this.options.pagination){var paginationResult=this._pagination();filtered.needHide=filtered.needHide.concat(paginationResult.needHide),filtered.needReveal=paginationResult.needReveal}if(this._bindArrangeComplete(),this._hideRevealItems(filtered),this._layout(),this.options.pagination)for(var i=0,l=this.filteredItems.length;i!==l;i++)this.filteredItems[i].isLayoutInstant=!1},Isotope.prototype._init=Isotope.prototype.arrange,Isotope.prototype._hideRevealItems=function(items){function hideReveal(){_this.reveal(items.needReveal),_this.hide(items.needHide)}var _this=this;this._isInstant?this._noTransition(hideReveal):hideReveal()},Isotope.prototype._getIsInstant=function(){var isInstant=void 0!==this.options.isLayoutInstant?this.options.isLayoutInstant:!this._isLayoutInited;return this._isInstant=isInstant,isInstant},Isotope.prototype._bindArrangeComplete=function(){function arrangeParallelCallback(){isLayoutComplete&&isHideComplete&&isRevealComplete&&_this.dispatchEvent("arrangeComplete",null,[_this.filteredItems])}var isLayoutComplete,isHideComplete,isRevealComplete,_this=this;this.once("layoutComplete",function(){isLayoutComplete=!0,arrangeParallelCallback()}),this.once("hideComplete",function(){isHideComplete=!0,arrangeParallelCallback()}),this.once("revealComplete",function(){isRevealComplete=!0,arrangeParallelCallback()})},Isotope.prototype._pagination=function(){this._lastFilter!==this.options.filter&&(this._lastFilter=this.options.filter,this.options.page=1),this.notPaginatedItems||(this.notPaginatedItems=this.filteredItems);var page=this.options.page,items=this.notPaginatedItems,startItemInPage=(page-1)*this.options.inPage,endItemInPage=startItemInPage+this.options.inPage-1,inPage=[],needHide=[],needReveal=[],totalPages=Math.ceil(items.length/this.options.inPage),pageChanged=this._lastPage!==page||this._totalPages!==totalPages;this._lastPage=page,this._totalPages=totalPages;for(var i=0,len=items.length;i!==len;i++){var item=items[i];i>=startItemInPage&&endItemInPage>=i?(inPage.push(item),item.isHidden&&(needReveal.push(item),item.isLayoutInstant=!0)):item.isHidden||needHide.push(item)}return this.filteredItems=inPage,pageChanged&&this.dispatchEvent("paginationUpdate",null,[page,totalPages,inPage]),{matches:inPage,needHide:needHide,needReveal:needReveal}},Isotope.prototype.page=function(pageNum){this.options.page=Math.max(1,Math.min(pageNum,this.totalPages())),this._hideRevealItems(this._pagination()),this._layout()},Isotope.prototype.nextPage=function(){this.page(this.options.page+1)},Isotope.prototype.previousPage=function(){this.page(this.options.page-1)},Isotope.prototype.lastPage=function(){this.page(this.totalPages())},Isotope.prototype.firstPage=function(){this.page(1)},Isotope.prototype.totalPages=function(){return this._totalPages},Isotope.prototype.currentPage=function(){return this.options.page},Isotope.prototype._filter=function(items){var filter=this.options.filter;filter=filter||"*";for(var matches=[],hiddenMatched=[],visibleUnmatched=[],test=this._getFilterTest(filter),i=0,len=items.length;len>i;i++){var item=items[i];if(!item.isIgnored){var isMatched=test(item);isMatched&&matches.push(item),isMatched&&item.isHidden?hiddenMatched.push(item):isMatched||item.isHidden||visibleUnmatched.push(item)}}return{matches:matches,needReveal:hiddenMatched,needHide:visibleUnmatched}},Isotope.prototype._getFilterTest=function(filter){return jQuery&&this.options.isJQueryFiltering?function(item){return jQuery(item.element).is(filter)}:"function"==typeof filter?function(item){return filter(item.element)}:function(item){return matchesSelector(item.element,filter)}},Isotope.prototype.updateSortData=function(elems){var items;elems?(elems=utils.makeArray(elems),items=this.getItems(elems)):items=this.items,this._getSorters(),this._updateItemsSortData(items)},Isotope.prototype._getSorters=function(){var getSortData=this.options.getSortData;for(var key in getSortData){var sorter=getSortData[key];this._sorters[key]=mungeSorter(sorter)}},Isotope.prototype._updateItemsSortData=function(items){for(var len=items&&items.length,i=0;len&&len>i;i++){var item=items[i];item.updateSortData()}};var mungeSorter=function(){function mungeSorter(sorter){if("string"!=typeof sorter)return sorter;var args=trim(sorter).split(" "),query=args[0],attrMatch=query.match(/^\[(.+)\]$/),attr=attrMatch&&attrMatch[1],getValue=getValueGetter(attr,query),parser=Isotope.sortDataParsers[args[1]];return sorter=parser?function(elem){return elem&&parser(getValue(elem))}:function(elem){return elem&&getValue(elem)}}function getValueGetter(attr,query){var getValue;return getValue=attr?function(elem){return elem.getAttribute(attr)}:function(elem){var child=elem.querySelector(query);return child&&getText(child)}}return mungeSorter}();Isotope.sortDataParsers={parseInt:function(val){return parseInt(val,10)},parseFloat:function(val){return parseFloat(val)}},Isotope.prototype._sort=function(){var sortByOpt=this.options.sortBy;if(sortByOpt){var sortBys=[].concat.apply(sortByOpt,this.sortHistory),itemSorter=getItemSorter(sortBys,this.options.sortAscending);this.options.pagination?this.notPaginatedItems.sort(itemSorter):this.filteredItems.sort(itemSorter),sortByOpt!=this.sortHistory[0]&&this.sortHistory.unshift(sortByOpt)}},Isotope.prototype._mode=function(){var layoutMode=this.options.layoutMode,mode=this.modes[layoutMode];if(!mode)throw new Error("No layout mode: "+layoutMode);return mode.options=this.options[layoutMode],mode},Isotope.prototype._resetLayout=function(){Outlayer.prototype._resetLayout.call(this),this._mode()._resetLayout()},Isotope.prototype._beforeLayout=function(items,isInstant){var mode=this._mode();mode._beforeLayout&&mode._beforeLayout(items,isInstant)},Isotope.prototype._getItemLayoutPosition=function(item){return this._mode()._getItemLayoutPosition(item)},Isotope.prototype._manageStamp=function(stamp){this._mode()._manageStamp(stamp)},Isotope.prototype._getContainerSize=function(){return this._mode()._getContainerSize()},Isotope.prototype.needsResizeLayout=function(){return this._mode().needsResizeLayout()},Isotope.prototype.resize=function(){this.isResizeBound&&this.needsResizeLayout()&&(this.options.resizeTransition?this.layout():this._noTransition(this.layout))},Isotope.prototype.appended=function(elems){var items=this.addItems(elems);if(items.length){var pagination=this.options.pagination,filteredItems=this._filterRevealAdded(items,!pagination);if(pagination){this.notPaginatedItems=this.notPaginatedItems.concat(filteredItems),this._resetLayout(),this._manageStamps();var paginateResult=this._pagination();this._hideRevealItems(paginateResult),this.layoutItems(this.filteredItems)}else this.filteredItems=this.filteredItems.concat(filteredItems)}},Isotope.prototype.prepended=function(elems){var items=this._itemize(elems);if(items.length){this._resetLayout(),this._manageStamps();var pagination=this.options.pagination,filteredItems=this._filterRevealAdded(items,!pagination);if(pagination){this.notPaginatedItems=filteredItems.concat(this.notPaginatedItems);var paginateResult=this._pagination();this._hideRevealItems(paginateResult),this.layoutItems(this.filteredItems)}else this.layoutItems(this.filteredItems),this.filteredItems=filteredItems.concat(this.filteredItems);this.items=items.concat(this.items)}},Isotope.prototype._filterRevealAdded=function(items,reveal){var filtered=this._filter(items);return this.hide(filtered.needHide),reveal!==!1&&(this.reveal(filtered.matches),this.layoutItems(filtered.matches,!0)),filtered.matches},Isotope.prototype.insert=function(elems){var items=this.addItems(elems);if(items.length){var i,item,len=items.length;for(i=0;len>i;i++)item=items[i],this.element.appendChild(item.element);var filteredInsertItems=this._filter(items).matches;for(i=0;len>i;i++)items[i].isLayoutInstant=!0;for(this.arrange(),i=0;len>i;i++)delete items[i].isLayoutInstant;this.reveal(filteredInsertItems)}};var _remove=Isotope.prototype.remove;return Isotope.prototype.remove=function(elems){elems=utils.makeArray(elems);var removeItems=this.getItems(elems);_remove.call(this,elems);var len=removeItems&&removeItems.length;if(len)for(var i=0;len>i;i++){var item=removeItems[i];utils.removeFrom(this.filteredItems,item)}},Isotope.prototype.shuffle=function(){for(var i=0,len=this.items.length;len>i;i++){var item=this.items[i];item.sortData.random=Math.random()}this.options.sortBy="random",this._sort(),this._layout()},Isotope.prototype._noTransition=function(fn){var transitionDuration=this.options.transitionDuration;this.options.transitionDuration=0;var returnValue=fn.call(this);return this.options.transitionDuration=transitionDuration,returnValue},Isotope.prototype.getFilteredItemElements=function(){for(var elems=[],i=0,len=this.filteredItems.length;len>i;i++)elems.push(this.filteredItems[i].element);return elems},Isotope}),/*! 
 * 
 ...
 **/
function($){function process(){check_lock=!1;for(var index=0,selectorsLength=selectors.length;selectorsLength>index;index++){var $appeared=$(selectors[index]).filter(function(){return $(this).is(":appeared")});if($appeared.trigger("appear",[$appeared]),$prior_appeared[index]){var $disappeared=$prior_appeared[index].not($appeared);$disappeared.trigger("disappear",[$disappeared])}$prior_appeared[index]=$appeared}}function add_selector(selector){selectors.push(selector),$prior_appeared.push()}var selectors=[],check_binded=!1,check_lock=!1,defaults={interval:250,force_process:!1},$window=$(window),$prior_appeared=[];$.expr[":"].appeared=function(element){var $element=$(element);if(!$element.is(":visible"))return!1;var window_left=$window.scrollLeft(),window_top=$window.scrollTop(),offset=$element.offset(),left=offset.left,top=offset.top;return top+$element.height()>=window_top&&top-($element.data("appear-top-offset")||0)<=window_top+$window.height()&&left+$element.width()>=window_left&&left-($element.data("appear-left-offset")||0)<=window_left+$window.width()?!0:!1},$.fn.extend({appear:function(options){var opts=$.extend({},defaults,options||{}),selector=this.selector||this;if(!check_binded){var on_check=function(){check_lock||(check_lock=!0,setTimeout(process,opts.interval))};$(window).scroll(on_check).resize(on_check),check_binded=!0}return opts.force_process&&setTimeout(process,opts.interval),add_selector(selector),$(selector)}}),$.extend({force_appear:function(){return check_binded?(process(),!0):!1}})}(function(){return"undefined"!=typeof module?require("jquery"):jQuery}()),/*! 
 * 
