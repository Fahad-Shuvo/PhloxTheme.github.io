...
/*! 
 * 
 * ================== js/libs/plugins/jquery.appear.js =================== 
 **/ 
...
/*!
 *  Averta JavaScript Tools - v1.0.0 (2016-11-04)
 *
 *  A collection of JavaScript files that used in multiple projects
 *
 *  Copyright (c) 2010-2016 Averta <info@averta.net> (www.averta.net)
 *  License: 
 */
...
;(function(){
    ...
    window.MSAligner = function(type , $container , $img, options ){
...
;(function ( $, window, document, undefined ) {
        ...
        '<div class="pswp__scroll-wrap">'+
            ...
            '<div class="pswp__ui pswp__ui--hidden">'+
                ...
                '<div class="pswp__top-bar">'+
                    ...
                    '<button class="pswp__button pswp__button--close" title="' + ( photoswipe_l10n.close || 'Close (Esc)' ) + '"></button>'+

                    '<button class="pswp__button pswp__button--share" title="' + ( photoswipe_l10n.share || 'Share' ) + '"></button>'+

                    '<button class="pswp__button pswp__button--fs" title="' + ( photoswipe_l10n.fullscreen || 'Toggle fullscreen' ) + '"></button>'+

                    '<button class="pswp__button pswp__button--zoom" title="' + ( photoswipe_l10n.zoom || 'Zoom in/out' ) + '"></button>'+
                ...
                '<button class="pswp__button pswp__button--arrow--left" title="' + ( photoswipe_l10n.previous || 'Previous (arrow left)' ) + '">'+
                '</button>'+

                '<button class="pswp__button pswp__button--arrow--right" title="' + ( photoswipe_l10n.next || 'Next (arrow right)' ) + '">'+
                '</button>'+
    ...
    $.extend( JQPhotoSwipe.prototype, {
        init: function () {
            this.$element.find( this.settings.target ).each( this._registerSlide.bind(this) );
        ...
        _registerSlide: function( index, item ) {
            ...
            var $item = $(item),
                slide = {
                    src   : $item.is('a') ? $item.attr( 'href' ) : ( $item.data( 'original-src' ) || $item.attr( 'src' ) ),
            ...
            if ( this.settings.titleMap ) {
                ...
                slide.title = $item.data( 'caption' ) || $item.attr( 'title' ) || $item.attr( 'alt' );
            }
        ...
        _openPhotoSwipe: function( index, fromURL ) {
            ...
            gallery = new PhotoSwipe( $pswp[0], options.ui, this.slides, options);
            gallery.init();
        }
    ...
    $.fn.photoSwipe = function ( options ) {
        ...
        if (options === undefined || typeof options === 'object') {
            ...
            this.each(function () {
                ...
                if (instance instanceof JQPhotoSwipe && typeof instance[options] === 'function') {
...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        ...
        _checkElementBoundaries: function( etp, wst, calc ) {
            etp = this.$containerPlaceHolder.offset().top;
            wst = wst || $window.scrollTop();
            calc = calc || 0;

            this.$parent = this.$parent || this.$element.parent().eq(0);
            var diff = this.$parent.offset().top + this.$parent.outerHeight(true) - (etp + wst + this.$element.outerHeight(true));

            if ( diff <= 0 ) {
                this.element.style[_jcsspfx + 'Transform'] = 'translateY(' + ( diff + calc ) + 'px)';
            } else if ( calc ) {
                this.element.style[_jcsspfx + 'Transform'] = 'translateY(' + calc + 'px)';
            } else {
                this.element.style[_jcsspfx + 'Transform'] = '';
...
;(function ( $, window, document, undefined ) {
    ...
    $.extend( Plugin.prototype, {
        init: function() {
            ...
            // disable default transiions in isotope
            this._isotope.options.hiddenStyle = {};
            this._isotope.options.visibleStyle = {};

            // store isotope on element
            this.$element.data( 'isotope', this._isotope ).addClass( this.settings.afterInitClass );
            ...
            this._currentFilter = this._isotope.options.filter;
        ...
        arrange: function( method, options ) {
            ...
            // check filter anad page
            if ( this._currentFilter === io.filter && this._currentPage === io.page ) {
                return;
            } else {
                this._currentPage = io.page;
                this._currentFilter = io.filter;
            }
            ...
            clearTimeout( this._hidingTimout );
            clearTimeout( this._revealingTimeout );

            this._hidingTimout = setTimeout( function(){
                self._instatnlyHideItems();
        ...
        _revealItems: function() {
            ...
            if ( !st.lazyload || this._isFilteredItemsLoaded() ) {
                if ( st.lazyload ) {
                    this._hideLoading();
                    this._isotope._noTransition(this._isotope.arrange);
                ...
                }.bind(this), st.revealTransitionDelay || 5 );
            } else {
                this._showLoading();
        ...
        ـinitFilters: function() {
            if ( this.settings.filters ) {
                ...
                this.$filters.find( 'li' ).on( 'click', function( e ){
                    ...
                    if ( filter.length ) {
                        if ( filter === 'all' ) {
                            self._isotope.options.filter = undefined;
                        } else {
                            self._isotope.options.filter = '.' + filter;
                        }
                    } else {
                        self._isotope.options.filter = undefined;
                    }

                    if ( !self._internalFilterChange ) {
        ...
        _updateSelectedFilter: function() {
            ...
            this.$filters.find( '[data-filter="' + (this._isotope.options.filter || 'all').replace('.' ,'') + '"] a' ).trigger( 'click' );
        },
        ...
        /**
         * updates the pagination contorl
        ...
        _readHash: function( arrange ) {
            ...
            io.filter = this._parseFilter( result[2] );
            io.page = this._checkPagePolicy( parseInt( result[3] ) );

            if ( !arrange || ( io.filter === oldFilter && io.page === oldPage ) ) {
                return;
            }
        ...
        _updateHash: function() {
            ...
            var hashStr = '/' + this.settings.slug + '/' + this._sanitizeFilter(this._isotope.options.filter) + '/' + this._isotope.options.page,
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if( response.success ) {
                        ...
                        postCounter         = $(response.data).filter('.aux-post-count').text();
                        // Remove aux-post-count block from response.data
                        var appendedData    = $(response.data).filter('.aux-ajax-item, .aux-date-label, style');
                        
                        // append new data by isotope insert | append method
                        if( this.$element.find('.aux-isotope-ready').length ) {
                            $(appendedData).each(function( key, value ) {
                                this.ajaxView.delay(80).queue(function (next) {
                                    if( $(value).hasClass( 'aux-image-box' ) ) {
                                        // update image alignment inside the tiles upon loadmore
                                        $(value).AuxinImagebox();
                                    }
                                    if( $(value).find( '.aux-media-video, .aux-media-audio' ).length ) {
                                        if( $(value).find( 'iframe' ).length ) {
                                            // Creating intrinsic ratios for videos
                                            $(value).fitVids({ customSelector: 'iframe[src^="http://w.soundcloud.com"], iframe[src^="https://w.soundcloud.com"]'});
                                        } else {
                                            // Call mediaelement player
                                            $(value).find('video,audio').mediaelementplayer();                                            
                                        }
                                    }                                                          
                                    $(this).isotope( 'insert', $(value) ).isotope( 'reloadItems' );
                                    next();
                                });                              
                            }.bind(this));                                 
                        } else {
                    ...
                    if( this.ajaxView.hasClass('aux-match-height') ) {
    ...
    $.extend(Plugin.prototype, {
        init: function() {
            ...
            if ( this.$element.find( '.' + this.settings.selected ).length === 0 ) {
                this.$current = this.$targets.eq(0).trigger( this.settings.event );
            }
...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        init: function() {
            ...
            if ( !MSAligner ){
                $.error( "MSAligner is not defined in this page, Auxin video box requires this library to perform correctly." );
            } else {
                this.aligner = new MSAligner( this.$element.data( 'fill' ) || 'fill' , this.$element, this.$video );
...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        init: function() {
            ...
            if ( !MSAligner ){
                $.error( "MSAligner is not defined in this page, Auxin image box requires this library to perform correctly." );
                return;
            }
            ...
            this.aligner = new MSAligner( this.settings.fill , this.$parent, this.$image, {
...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "AuxinAnimateAndRedirect",
        defaults = {
            ...
            scrollFixTarget     : 'body',                // The containt that checks for scroll top value on animation
            ...
            noAnimate           : 'aux-no-page-animate', // Targets by this class don't animate the page
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            if ( st.noAnimate && this.$element.hasClass( st.noAnimate ) ) { return; }
...
/*!
 *  Master Carousel - v1.1.6 (2017-06-14)
 *  http://www.averta.net
 *
 *  A flexible, touch and lightweight carousel script for Auxin
 *
 *  Copyright (c) 2010-2017 averta <www.averta.net>
...
(function ( $, window, document ) {
	...
	$.extend(MasterMenuPlugin.prototype, {
		...
		/**
		 * Prepare intraction events based on new menu type
		 * @private
		 */
