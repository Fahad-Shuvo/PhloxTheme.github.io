...
?>
    <div id="message" class="updated auxin-message">
        <p><?php _e( 'In order to import demo content and add more features to Phlox theme, it is recommended to install <strong>Phlox Core Plugin</strong>.', 'phlox' ); ?></p>
        <p class="submit"><a href="<?php echo esc_url( self_admin_url( 'plugin-install.php?s=phlox+core&tab=search&type=term' ) ); ?>" class="button-primary"><?php _e( 'Install Core Plugin', 'phlox' ); ?></a> <a class="button-secondary skip" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'aux-hide-core-plugin-notice', 'install' ), 'auxin_hide_notices_nonce', '_notice_nonce' ) ); ?>"><?php _e( 'Skip', 'phlox' ); ?></a></p>
    </div>
<?php
}
