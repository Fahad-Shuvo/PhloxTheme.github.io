...
function auxin_change_custom_logo_attributes( $attr, $attachment, $size ){
    if( ! empty( $attr['class'] ) && 'custom-logo' == $attr['class'] ){
        $attr['class'] .= ' aux-logo-image aux-logo-light';
...
function auxin_get_page_background_style( $css, $post ){
    ...
    if( auxin_is_true( auxin_get_post_meta( $post->ID, 'aux_custom_bg_show', true ) == 1 ) ){
        ...
        $pattern = auxin_get_post_meta( $post->ID, 'aux_custom_bg_pattern' );
        if( ! empty( $pattern ) ){
...
function auxin_body_class( $classes ) {
    ...
    if ( auxin_get_option( 'site_header_top_sticky', true )
        && 'vertical' !==  auxin_get_option( 'site_header_navigation_layout', 'horizontal' ) ) {
    ...
    if( auxin_get_option('site_footer_is_sticky') ){
    ...
    if ( 'vertical' === auxin_get_option( 'site_header_navigation_layout' ) ){
...
function auxin_add_goto_top_btn(){
    ...
    $goto_top = '<div class="aux-goto-top-btn aux-align-btn-'. esc_attr( auxin_get_option( 'goto_top_alignment' ) ) .'" data-animate-scroll="'. esc_attr( auxin_get_option('goto_top_animate') ) .'">'.
...
function auxin_add_page_preload_animation(){
    ...
    if ( auxin_get_option('page_preload_enable') ) {
        ...
        ?>
    <div id="pagePreloadProgressbar" class="aux-no-js" <?php echo $progressbar_style; ?> ></div>
        <?php
        }
    ...
    if ( auxin_get_option('page_animation_nav_enable') ) {
        // insert a division for overlay page animations
        echo '<div class="aux-page-animation-overlay"></div>';
    }
}
...
function auxin_add_site_header_attributes( $attributes ){
    ...
    $attributes['class'] .= 'aux-territory aux-' . esc_attr( auxin_get_option('site_header_width') ) . '-container ';
    $attributes['class'] .= 'aux-header-' . esc_attr( auxin_get_option('site_header_color_scheme') . ' ' ) ;
    $attributes['class'] .=  auxin_get_option('site_header_border_bottom') ? 'aux-add-border ' : '';
    $attributes['class'] .= 'logo-left-menu-right-over' === auxin_get_top_header_layout() ? ' aux-over-content ' : '';

    if ( auxin_is_true ( $overlay_header ) ) {

        if ( auxin_is_true ( $display_top_header ) ) {

            $attributes['class'] .= 'aux-overlay-with-tb ' ;

        } else{

            $attributes['class'] .= 'aux-overlay-header ' ;
    ...
    if( auxin_get_option( "site_header_top_sticky" ) ){
        $attributes['data-color-scheme'] = auxin_get_option('site_header_color_scheme');
