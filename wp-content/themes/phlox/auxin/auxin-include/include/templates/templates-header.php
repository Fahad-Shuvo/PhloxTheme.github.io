...
function auxin_main_title_preset_args( $defaults, $post = null ){
    ...
    if ( is_archive() || is_search() ) {
        ...
        $show_titlebar = auxin_get_post_meta( $post->ID, 'aux_title_bar_show', "default" );
        $titlebar_args['show_titlebar'] = ( 'default' === $show_titlebar ) ? auxin_get_option( $post->post_type . '_title_bar_show' ) : $show_titlebar;
    }
    ...
    if( $titlebar_args['show_titlebar'] ){
        ...
        if ( 'default' === $show_titlebar || ( 'yes' === $show_titlebar &&  'default' === auxin_get_post_meta( $post->ID, 'aux_title_bar_preset', 'default' ) ) ) {
...
if( ! function_exists( 'auxin_the_main_title_section' ) ){
    ...
    function auxin_the_main_title_section( $args = array() ){
        ...
        if( is_home() ){
            ...
            $defaults['title'] = __( 'Results for: ', 'phlox') . '<span>'. get_search_query() . '</span>';
                ...
                <div class="aux-container" >
                    ...
                    <?php } ?>
                            <section class="page-title-group" <?php echo $title_group_inline_style; ?>>
                                ...
                                <h3 class="page-subtitle" ><?php echo esc_html( $args['subtitle'] ); ?></h3>
                                <?php } ?>
                            </section>
                            ...
                            <div class="page-title-meta aux-single-inline-meta">
                                ...
                                <span class="author vcard">
                                    <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author" title="<?php echo esc_url( sprintf( __( 'View all posts by %s', 'phlox' ), get_the_author() ) ); ?>" >
                                        <?php the_author(); ?>
                                    </a>
                                </span>
...
if( ! function_exists( 'auxin_the_header_slider_section' ) ){
    ...
    function auxin_the_header_slider_section(){
        ...
        // $wrapper_start_tag = '<div id="site_topslider" class="'.$container_class.'" ><div class="aux-wrapper"><div class="aux-container '. esc_attr( $fold_class ) .'">';
        // @endif

        $wrapper_start_tag = '<div id="site_topslider" class="'.$container_class.'" ><div class="aux-wrapper"><div class="aux-container">';
...
function auxin_get_the_archive_slider( $post_type = 'post', $request_for_location = 'block' ){
    ...
    if( is_tax() || is_post_type_archive('portfolio') || is_post_type_archive('news') || ( auxin_is_plugin_active( 'woocommerce/woocommerce.php') && ( is_shop() || is_product_category() || is_product_tag() ) ) ) {
                return'';
        }
...
function auxin_get_header_layout( $layout = '' ){
    ...
    if ( 'vertical' === auxin_get_option( 'site_header_navigation_layout', 'horizontal' ) ) {
        $layout = 'vertical';
    }
...
?>
        ...
        case 'vertical':
            $items_align = 'aux-vertical-items-' . auxin_get_option('site_vertical_header_items_align', 'left');
            $search_border = auxin_is_true( auxin_get_option('site_vertical_header_search_border' , '1') ) ? '' : 'aux-search-no-border' ;
            $icon_size  = auxin_get_option('site_vertical_header_social_icon', 'small');
            ?>
            <!-- burger -->
            <div id="nav-burger" class="aux-burger-box aux-end aux-phone-on aux-middle" data-target-panel="<?php echo esc_attr( $mobile_menu_position ); ?>" data-target-menu="overlay" >
                <div class="aux-burger <?php echo esc_attr( $site_mobile_header_toggle_button_style ); ?>"><span class="mid-line"></span></div>
            </div>
            <div class="aux-vertical-menu-side aux-header aux-header-elements-wrapper <?php echo esc_attr( $items_align ) ;?>">
                <!-- ribbon bar -->
                <div class="aux-vertical-menu-elements aux-header-elements ">
                    ...
                    <!-- menu -->
                    <div class="aux-menu-box aux-phone-off aux-auto-locate aux-start aux-tablet-center'; ?>" data-tablet=".aux-header .secondary-bar-2">
                    <?php
                        /* The menu assiged to the primary position is the one used.  If none is assigned, the menu with the lowest ID is used.*/
                        wp_nav_menu( array(
                            'container_id'   => 'master-menu-main-header',
                        ...
                        auxin_wc_add_to_cart( array( 'title' => __( 'Shopping Cart', 'phlox' ) ) );
                    ?>

                    </div>
                    <?php if ( auxin_is_true( auxin_get_option( 'site_vertical_menu_footer_display' ) ) ) : ?>
                        ...
                        <footer class="aux-vertical-menu-footer aux-phone-off">
                            ...
                            <?php
                            if ( auxin_is_true( auxin_get_option( 'site_vertical_menu_socials', true ) ) ) {
...
function auxin_get_custom_logo_image(){
    ...
    $custom_logo_classes  = 'custom-logo-link aux-logo-anchor aux-middle';
    $custom_logo_classes .= has_custom_logo() ? ' aux-has-logo' : '';

    // append new class names and inline style to logo anchor
    $logo_markup = str_replace( 'custom-logo-link', $custom_logo_classes, $logo_markup );
...
if( ! function_exists('auxin_the_top_header_section') ){
    ...
    function auxin_the_top_header_section(){
        ...
        if( 'horizontal' === auxin_get_option( 'site_header_navigation_layout', 'horizontal' ) && ( auxin_get_option('show_topheader') || is_customize_preview() ) ) {
            $classes  = "aux-". auxin_get_option('site_header_width'). "-container";
 ...
 * @return void
 */
if( ! function_exists('auxin_the_main_header_section') ){
    ...
    function auxin_the_main_header_section(){
        ...
        ?>
        <header id="site-header" <?php auxin_dom_attributes('site_header'); ?>>
            <div class="aux-wrapper">

            <?php if( 'vertical' === auxin_get_option( 'site_header_navigation_layout' ) ){
