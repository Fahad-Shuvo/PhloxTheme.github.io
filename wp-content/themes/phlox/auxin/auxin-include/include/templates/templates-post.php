...
if( ! function_exists( 'auxin_comment' ) ){
    ...
    function auxin_comment( $comment, $args, $depth ) {
       ...
       $author_indicator = $comment->user_id === $post->post_author ? '<span class="author-indicator">' . esc_html__('Author', 'phlox' ) . '</span>' : '';
       ?>
       <li>
...
function auxin_single_page_navigation( $args = array() ){
    ...
    }elseif( 'thumb-no-arrow' == $args['skin'] ) {
        $css_class     = 'nav-skin-thumb-arrow no-arrow';
        $args['skin']  = 'thumb-arrow';
    }elseif( 'thumb-arrow-sticky' == $args['skin'] ) {
    ...
    <nav class="aux-next-prev-posts <?php echo esc_attr( $css_class ); ?>">
        ...
        <section class="np-prev-section <?php echo $nav_image ? 'has-nav-thumb':''; ?>"  <?php echo ( $container_style ) ;?> >
            <a href="<?php echo get_permalink( $prev_post->ID ); ?>">
                ...
                <p class="np-nav-text"><?php echo esc_attr( $args['prev_text'] ); ?></p>
                <?php if( $args['show_title'] && ! ( 'classic' == $args['skin'] ) ){ ?>
        ...
        <?php if ( 'classic-title' == $args['skin'] || 'classic' == $args['skin'] ){ ?>
        ...
        <section class="np-next-section <?php echo $nav_image ? 'has-nav-thumb':''; ?>" <?php echo ( $container_style ) ;?> ">
            <a href="<?php echo get_permalink( $next_post->ID ); ?>">
                ...
                <p class="np-nav-text"><?php echo esc_attr( $args['next_text'] ); ?></p>
                <?php if( $args['show_title'] && ! ( 'classic' == $args['skin'] ) ){ ?>
    ...
    function auxin_get_content_main_css_classes( $class = '' ){
        ...
        if( is_singular() ){
            ...
            if( $post ){
                ...
                if( 'default' === $show_top_margin = auxin_get_post_meta( $post->ID, 'show_content_top_margin', 'default' ) ){
        ...
        switch ( $element_info['element'] ) {
            ...
            case 'menu':
                ...
                if( $locations['footer'] ){
                    ...
                    echo '<div class="'.$align_css_class.'">' . __( 'Admin notice: Menu for "footer" location not found.', 'phlox' ) . '</div>';
                }
                break;
...
function auxin_wc_add_to_cart( $args = array() ){
    ...
    ?>
        ...
        <div class="aux-cart-wrapper <?php echo esc_attr( $args['css_class'] ); ?>">
            <a class="aux-cart-contents aux-action-on-<?php echo esc_attr( $args['action_on'] ); ?> auxicon-shopping-cart-1-1" href="<?php echo esc_url( $args['cart_url'] ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'phlox' ); ?>">
                <?php echo isset( $args['title'] ) ? $args['title'] : '';
                 if ( $count > 0 ) echo '<span>' . $count . '</span>'; ?>
            </a>
            <?php if( ! is_cart() && ! is_checkout() ) : ?>
                <?php if( $count > 0 ) { ?>
                    <div class="aux-card-dropdown <?php echo esc_attr( $args['dropdown_class'] ); ?>">
                        ...
                        ?>
                            <div class="aux-card-item" data-product-id="<?php echo esc_attr( $values['product_id'] ); ?>">
                                ...
                                <div class="aux-card-item-details">
                                    ...
                                    <a href="<?php echo esc_url( $woocommerce->cart->get_remove_url( $item ) ); ?>" class="aux-remove-cart-content aux-svg-symbol aux-small-cross <?php echo esc_attr( $args['color_class'] ); ?>" data-verify_nonce="<?php echo wp_create_nonce( 'remove_cart-' . $values['product_id'] ); ?>" data-product_id="<?php echo esc_attr( $values['product_id'] ); ?>"></a>
                                </div>
                            </div>
                        <?php } ?>
                        </div>
                        <div class="aux-card-checkout">
                            <div class="aux-card-final-amount">
                            <?php
                                echo __( 'SUB TOTAL', 'phlox' ) . $woocommerce->cart->get_cart_subtotal();
                            ?>
                            </div>
                            <div class="button-wrapper">
                                <a target="_blank" href="<?php echo $woocommerce->cart->get_checkout_url(); ?>" class="aux-button aux-large <?php echo esc_attr( $args['color_class'] ); ?> aux-uppercase"><span class="aux-overlay"></span><span class="aux-text"><?php esc_attr_e( 'Proceed To Checkout', 'phlox' ); ?></span></a>
                                <div class="clear"></div>
                                <a target="_blank" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="aux-button aux-large aux-outline aux- <?php echo esc_attr( $args['color_class'] ); ?> aux-outline aux-uppercase"><span class="aux-overlay"></span><span class="aux-text"><?php esc_attr_e( 'View Cart', 'phlox' ); ?></span></a>
                            </div>
                        </div>
                    </div>
                <?php } else { ?>
...
function auxin_paginate_list( $args = '' ) {
    ...
    if ( $args['first_last'] && $current && 1 < $current ) :
         ...
         */
        $page_links[] = '<li class="first"><a class="page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['first_text'] . '</a></li>';
    endif;
    ...
    if ( $args['prev_next'] && $current && 1 < $current ) :
         ...
         * @param string $link The paginated link URL.
         */
        $page_links[] = '<li class="prev"><a class="page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['prev_text'] . '</a></li>';
    endif;
...
function auxin_get_post_format_media( $post, $settings = array() ){
    ...
    switch ( $the_post_format ) {
        ...
        case 'gallery':
            ...
            $args['the_media']  = '<div class="aux-no-js aux-media-frame aux-media-gallery aux-lightbox-in-slider master-carousel-slider" data-navigation="perpage" data-loop="true" data-space="0" data-auto-height="true" data-empty-height="'. esc_attr( $size['height'] ) .'">';
            ...
            // dont generate media if Display Featured image in customizer has been off in blog pages

            if( !( auxin_is_true( auxin_get_option( 'blog_archive_show_featured_image', 1 ) ) && 'archive' === $request_from ) ){
                $args['the_media'] = '';
            }

            if ( 'default' === $title_bar_show = auxin_get_post_meta( $post->ID, 'aux_title_bar_show', 'default' ) ){
                $title_bar_show = auxin_get_option( $post->post_type . '_title_bar_show' );
            }

            // Don't display post title if title bar is enable to prevent duplicated title in single page
            if( 'single' === $request_from && ( ! auxin_is_true( $title_bar_show ) && auxin_get_option( 'post_single_title_show_over_content' ) ) ){
        ...
        case 'image':
            ...
            if ( 'default' === $title_bar_show = auxin_get_post_meta( $post->ID, 'aux_title_bar_show', 'default' ) ){
                $title_bar_show = auxin_get_option( $post->post_type . '_title_bar_show', 'no' );
            }

            // Don't display post title if title bar is enable to prevent duplicated title in single page
            if( 'single' === $request_from && auxin_is_true( $title_bar_show ) ){
                $args['show_title'] = false;
            }
            ...
            if( 'archive' === $request_from ){
                ...
                $show_featured_image_meta_field = auxin_get_post_meta( $post->ID, 'blog_archive_show_featured_image', 'auto' );
                if( 'auto' == $show_featured_image_meta_field ){
            ...
            } elseif( 'single' === $request_from ){

                // get metafield featured image visibility option
                $show_featured_image_meta_field = auxin_get_post_meta( $post->ID, 'blog_single_show_featured_image', 'default' );
                if( ! ( 'default' == $show_featured_image_meta_field || auxin_is_true( $show_featured_image_meta_field ) ) ){
                    break;
                }
