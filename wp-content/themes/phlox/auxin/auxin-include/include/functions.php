    ...
    if( ! function_exists( 'auxin_get_the_trim_excerpt' ) ){
        ...
        function auxin_get_the_trim_excerpt( $post_id = null, $char_length = null, $exclude_strip_shortcode_tags = null, $skip_more_tag = false, $wrap_excerpt_with = '' ) {
            ...
            if ( ! $skip_more_tag && preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
                ...
                $more_link = sprintf( ' <a href="%s#more-%s" class="more-link">%s</a>', esc_url( get_permalink() ), esc_attr( $post->ID ), $excerpt_more );
                $more_link = apply_filters( 'the_content_more_link', $more_link, $excerpt_more , 'auxin_trim_excerpt' );
...
function auxin_the_post_thumbnail( $post_id = null, $width = null , $height = null, $crop = null , $quality = 100, $upscale = false ) {
    ...
    function auxin_get_the_responsive_attachment( $attachment_id = null, $args = array() ) {
        ...
        foreach ( $srcset_sizes as $srcset_size ) {
            ...
            $srcset_size['width' ] = empty( $srcset_size['width' ] ) ? null : $srcset_size['width' ];
            $srcset_size['height'] = empty( $srcset_size['height'] ) ? null : $srcset_size['height'];

            $attr_srcset .= auxin_get_the_resized_attachment_src( $attachment_id, $srcset_size['width'] , $srcset_size['height'], $crop, $quality, $upscale );
            $attr_srcset .= ' '. round( $srcset_size['width'] ).'w,';
        }

        if ( $original_src ) {
            $original_size_img = wp_get_attachment_image_src( $attachment_id, 'full' );
            $attr_srcset .= $original_size_img[0] . ' ' . $original_size_img[1] . 'w';
        } else {
            $attr_srcset =  rtrim( $attr_srcset, ',' );
        }

        // crop the main image
        $src = auxin_get_the_resized_attachment_src( $attachment_id, $size['width'] , $size['height'], $crop, $quality, $upscale );
        if( empty( $src ) ) return '';
        ...
        // replace src and srcset if the preloadable is true
        if ( $preloadable ) {
            ...
            foreach ( $models as $model ) {
                ...
                if( is_string( $model ) ){
                    if( function_exists( $model ) ){
                        ...
                        echo auxin_error( sprintf( __( 'The function name "%s" not found for Metabox hub "%s".', 'phlox' ), $model, $args['hub_id'] ) );
                    }
                }
...
function auxin_get_last_post_id( $args ){
    ...
    if ( $the_query->have_posts() ) {
        $the_query->the_post();
        $last_post_id = $the_query->post->ID;
    }
