...
class Auxin_Metabox_Hub extends Auxin_Metabox{
    ...
    public function display_meta_box() {
        ...
        echo '<div class="av3_container" >';
            printf('<div class="form-table meta-box axi-metabox-hub axi-metabox-container %s">', $layout_mode);
            ...
            foreach ( $this->models as $model_group_id => $model_group ) {
                foreach( $model_group as $model ) {
                    ...
                    $tabs_markup  .= '<li class="'. esc_attr( $tab_css_class ) .'"><a href="#'.esc_attr( $tab_id ).'">'. esc_html( $model->title ) .'</a></li>';
            ...
            echo   '</div>',
            '<div class="axi-metabox-loading"><span class="spinner is-active"></span><span class="axi-metabox-loading-label">'.__('Loading options..', 'phlox' ).'</span></div>',
        '</div>';
