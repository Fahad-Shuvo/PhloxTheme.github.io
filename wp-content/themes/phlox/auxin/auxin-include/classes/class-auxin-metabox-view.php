...
class Auxin_Metabox_View extends Auxin_Metabox_Model{
    ...
    public function get_field_upload( $field ){
        ...
        $output = sprintf( $this->section_row_start, $field['class_names'], $field['attributes'] ).
            ...
            $output .=  '<div class="imgHolder">'.
                        ...
                        '<strong title="' . esc_attr__( "Remove image", 'phlox' ) . '" class="close">X</strong>'.
                        '<img alt src="" /></div>'.
                        '<p >'. $field['description'].'</p>'.
                ...
                '<div class="section-row-right" >'.
                    '<div class="axi-attachmedia-wrapper" >'.
                        sprintf( '<input type="text" name="%1$s" id="%1$s" value="%2$s"
                                 ...
                                 esc_attr__('Add Image', 'phlox' ), esc_attr__('Add Image', 'phlox' ), esc_attr__('Select Image', 'phlox' )
                                ).
                    '</div>'.
                    '<p >'. $field['description'].'</p>'.
                '</div>'.
                ...
                '<div class="section-row-right" >'.
                    '<div class="axi-attachmedia-wrapper" >'.
                        sprintf( '<input type="text" name="%1$s" id="%1$s" value="%2$s"
                                 ...
                                 >',
                                 $field['id'], $field['value'],
                                 esc_attr__('Add Image(s)', 'phlox' ), esc_attr__('Add Image', 'phlox' ), esc_attr__('Select Image', 'phlox' )
                                ).
                    '</div>'.
                    '<p >'. $field['description'].'</p>'.
                '</div>'.
                ...
                '<div class="section-row-right" >'.
                    '<div class="axi-attachmedia-wrapper" >'.
                        sprintf( '<input type="text" name="%1$s" id="%1$s" value="%2$s"
                                 ...
                                 >',
                                 $field['id'], $field['value'],
                                 esc_attr__('Add Audio', 'phlox' ), esc_attr__('Add Audio', 'phlox' ), esc_attr__('Select Audio', 'phlox' )
                                ).
                    '</div>'.
                    '<p >'. $field['description'].'</p>'.
                '</div>'.
                ...
                '<div class="section-row-right" >'.
                    '<div class="axi-attachmedia-wrapper" >'.
                        sprintf( '<input type="text" name="%1$s" id="%1$s" value="%2$s"
                                 ...
                                 >',
                                 $field['id'], $field['value'],
                                 esc_attr__('Add Video', 'phlox' ), esc_attr__('Add Video', 'phlox' ), esc_attr__('Select Video', 'phlox' )
                                ).
                    '</div>'.
                    '<p >'. $field['description'].'</p>'.
                '</div>'.
    ...
    public function get_background_child_fields( $field ){
        ...
        $bg_fields['pattern'] = array(
            ...
            'choices'       => auxin_get_background_patterns( array( '' => array( 'label' =>__('None', 'phlox'), 'image' => AUXIN_URL . 'images/visual-select/none-pattern.svg' ) ), 'before' ),
            'default'   => '',
            'type'      => 'radio-image'
        );
    ...
    public function print_dependencies(){
        ...
        printf( 'auxin.metabox.%1$s.presets = %2$s;</script>', esc_js( $this->id ), wp_json_encode( $this->presets_list ) );
    }
    ...
    protected function update_field( $field, $post = null ){
        ...
        if ( $allow_save ) {
            ...
            // if its internal uploaded file, convert url to relative url
                ...
                update_post_meta( $post->ID, $field['id'], maybe_serialize( $new ) );
