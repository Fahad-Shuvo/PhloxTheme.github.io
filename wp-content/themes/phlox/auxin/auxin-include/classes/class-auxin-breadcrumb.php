...
class Auxin_Breadcrumb {
        ...
        if( is_single() ) {
            ...
            if( $this_page_type == "news" ) {
                ...
                if ( 'page' == $show_on_front && $page_for_posts ) {
                    ...
                    $blog_title  = esc_html_x( 'Blog', 'The blog title in breadcrumb', 'phlox' );
                }
                ...
                if( $nav_type == 'breadcrumb' ){
                    ...
                    $list_page = sprintf( '<a href="%s">%s</a>', get_post_type_archive_link( get_post_type() ), esc_html__( 'show all', 'phlox' ) );
    ...
    public function create_crumb( $title = '', $link = '', $sep = '' ){
        if( $title ) {
            ...
            $trimmed_title = esc_html( auxin_get_trimmed_string( $title, 30, "..." ) );
    ...
    public function get_next_prev_post_link( $start_tag = '', $end_tag = '', $sep = '', $middle_tag = '' ){
        ...
        $output  = $start_tag;
        $output .= get_previous_post_link( '%link', esc_html_x( 'previous', 'navigate to previous page', 'phlox') );
        $output .= $sep;
        $output .= ! empty( $middle_tag ) ? $middle_tag . $sep : '';
        $output .= get_next_post_link( '%link', esc_html_x( 'next', 'navigate to next page', 'phlox') );
        $output .= $end_tag;
